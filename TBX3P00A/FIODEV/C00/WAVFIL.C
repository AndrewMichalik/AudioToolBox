/************************************************************************/
/* Wave File Routines (DOS): WavFil.c                   V2.00  07/15/94 */
                ...
                DWORD ulSmpFrq, VISMEMHDL FAR *phwfChkFmt, MMCKINFO FAR *lpck)
{
    ...
    switch (usPCMTyp) {
        case DLGPCM004:
            pibChkFmt->mwMSW.afWEX.wfx.wFormatTag = WAVE_FORMAT_OKI_ADPCM;
    ...
    /********************************************************************/
    /********************************************************************/
    if (sizeof (PCMWAVEFORMAT) != mmioWrite (hFilHdl, 
        (LPSTR) &pibChkFmt->mwMSW.afWEX.wfx, sizeof (PCMWAVEFORMAT)))
        return (MMIOERR_CANNOTWRITE);
    if (usRetCod = mmioAscend (hFilHdl, lpck, 0)) return (usRetCod);
...
WORD PASCAL FAR WIOOutEnd (HMMIO hFilHdl, VISMEMHDL FAR *phwfChkFmt, MMCKINFO FAR *lpck)
{
    ...
    /********************************************************************/
//  ajm 05/25/96
//  ulRifLen = FilGetPos (hFilHdl) - 8;
    ulRifLen = mmioSeek (hFilHdl, 0L, SEEK_CUR) - 8;
    if (usRetCod = mmioAscend (hFilHdl, lpck, 0)) return (usRetCod);
